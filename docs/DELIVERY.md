...
## 과학 분류·설명 수정 (2026-09-15 재개)

최신 작업은 `molecule-studio-science-brief-20260914`입니다. CO₂ 카보닐 분류, 수소 결합 지표 정의, 모델/이상각/참고값 구분, 분광 범위 표현, 한국어 오류·컬렉션 호환성을 수정했습니다. 화학 19/19, 현재 생산 빌드 및 로컬 브라우저 23/23, 독립 화면/PNG 검토를 통과했습니다. 항목별 상태·입력별 결과·게시 상태·제한은 [SCIENCE_FIX_REPORT.md](SCIENCE_FIX_REPORT.md)에 기록합니다. 아래 내용은 이전 작업의 인계 이력입니다.

## 뷰어 안내와 기본 이름 수정

후속 작업 ID는 `molecule-studio-viewer-copy-20260914`입니다. 하단의 중복 조작 안내를 제거하고 뷰어 내부 안내 하나만 유지했습니다. 모바일에서 내부 최소 높이 때문에 안내가 잘리던 문제도 수정했습니다. 기본 이름은 뷰어·정보 패널·컬렉션·학습 카드에서 **분자(Molecule)**로 표시하며 실제 분자 이름과 저장 데이터는 유지합니다.

생산 빌드와 로컬 데스크톱(1440px)·모바일(390px) 검사를 통과했습니다. 벤젠 기본 제목, 에탄올 이름 유지, 안내 개수·영역 내 배치·줄바꿈을 확인했고 독립 화면 검토도 통과했습니다. 수정 전 모바일 잘림 화면과 최종 검사 기록은 `evidence/viewer-copy/`, 이번 계약 실행은 `runs/c607ddc354cb45419063501c4c3f1df8/`에 남깁니다. 연결된 Vercel 배포의 최종 확인 결과는 같은 증거 폴더의 `completion-evidence.json`에 기록합니다.

## 원자 라벨과 주요 원소 확장

후속 작업 ID는 `molecule-studio-labels-elements-20260914`입니다. 3D 도구 모음에 **원자 라벨** 체크박스를 추가해 식별 ID·원소 기호를 표시하거나 숨깁니다. 기본값은 표시이며 분자를 바꿔도 사용 중 선택을 유지합니다. 원자 자체와 측정 결과는 남고, 기존 **H 표시**와 독립적으로 동작합니다. 라벨 전환으로 카메라나 WebGL 캔버스를 다시 만들지 않습니다.

2D 편집기는 H·B·Si·I를 추가해 C·H·N·O·B·Si·P·S·F·Cl·Br·I의 12개 원소를 제공합니다. 한글 원소명과 선택 상태를 표시하며 H 글자 대비와 좁은 화면의 줄바꿈을 조정했습니다. 기존 RDKit 계산·VSEPR 지원 범위는 유지합니다.

TypeScript·생산 빌드 및 로컬 브라우저 15개 검사를 통과했습니다. 라벨 숨김 중 직접 원자 클릭·거리 측정, H/B/Si/I의 실제 그리기→API→3D, 모바일 토글, PNG 내용을 검사했습니다. 첫 검사의 잘못된 원자 선택자는 수정했고 실패 기록도 보존했습니다. 검증 증거는 `evidence/labels-elements/`, 계약 실행 기록은 `runs/df0333d6eab84c0fb958b802ef64b07f/`에 남깁니다.
